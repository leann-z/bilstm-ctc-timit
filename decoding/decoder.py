...
def decode(model, args, json_file, char=False):
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
          log_probs = torch.nn.functional.log_softmax(model(inputs), dim=-1)

          # apply blank penalty (in log space)
          if hasattr(args, "blank_penalty") and args.blank_penalty > 0:
            log_probs[:, :, blank_idx] -= args.blank_penalty

          outputs = torch.argmax(log_probs, dim=-1).transpose(0, 1)
